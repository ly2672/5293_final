...
from llm_utils import generate_cover_letter, evaluate_resume_with_gpt
...
if input_mode == "Freeform (text box)":
    ...
    if st.button("Generate Resume"):
        with st.spinner("Processing..."):
            ...
            st.session_state["resume_text"] = resume  # ✅ Store resume
        st.subheader("Generated Resume")
        st.markdown(f"```\n{resume}\n```")

    if st.button("📊 Evaluate Resume (GPT)"):
        if "resume_text" in st.session_state:
            with st.spinner("Evaluating resume with GPT..."):
                result = evaluate_resume_with_gpt(st.session_state["resume_text"])
            st.subheader("🔎 GPT Resume Evaluation")
            st.markdown(f"```\n{result}\n```")
        else:
            st.error("❗ Please generate the resume first.")
...
# ========== STRUCTURED INPUT ==========
    ...
    with st.form("structured_form"):
        ...
        if submitted:
            ...
            resume = generate_resume(structured)
            st.session_state["resume_text"] = resume  # ✅ Store for GPT grading

            st.subheader("Generated Resume")
            st.markdown(f"```\n{resume}\n```")

    if st.button("📊 Evaluate Resume (GPT)", key="evaluate_structured"):
        if "resume_text" in st.session_state:
            with st.spinner("Evaluating resume with GPT..."):
                result = evaluate_resume_with_gpt(st.session_state["resume_text"])
            st.subheader("🔎 GPT Resume Evaluation")
